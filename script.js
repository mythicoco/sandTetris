...
function lower(i, board) {
    ...
    if (board[i] === 1) {if (board[i+index] === 0) {board[i] = 0;board[i+index] = 1} else {if (board[i+index-1] !== 0 && isEmptyFor(i,1,board) && board[i+index+1] === 0 && c !== index-1) {;board[i] = 0;board[i+index+1] = 1} else if (board[i+index-1] === 0 && isEmptyFor(i,-1,board) && board[i+index+1] !== 0 && c !== 0) {board[i] = 0;board[i+index-1] = 1} else if ((board[i+index-1] === 0 && isEmptyFor(i, -1, board) && c !== 0) || (board[i+index+1] === 0 &&  isEmptyFor(i, 1, board) && c !== index-1)) {if (isEmptyFor(i,-1,board)) {board[i] = 0;board[i+index-1] = 1} else if (isEmptyFor(i,1,board)) {board[i] = 0;board[i+index+1] = 1}} else if (board[i+index-1] === 1 && board[i+index+1] === 1) return board;}
        ...
        for(let x=1;x<=50;x++) {
            ...
            else if ((board[i+index-x] === 0 &&  isEmptyFor(i, -x, board) &&  c >= x) || (board[i+index+x] === 0 && c <= index-(x+1) && isEmptyFor(i, x, board))) {if(isEmptyFor(i,-x,board)) {board[i] = 0;board[i+index-x] = 2;return board} else if (isEmptyFor(i,x,board)) {board[i] = 0;board[i+index+x] =2;return board}}
                else if (board[i+index+x] === 0 && c <= index-1-x && isEmptyFor(i, x, board)) {board[i] = 0;board[i+index+x] = 2;return board} else if (board[i+index-x] === 0 && c >= x && isEmptyFor(i, -x, board)) {board[i] = 0;board[i+index-x] = 2;return board}
        }}; return board
}
